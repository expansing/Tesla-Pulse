...
5. **Combine recent windows.** Tesla Pulse retains up to the newest 12 accepted windows and calculates the **ΔSOC-weighted average** of the 5 most recent as the SOC-window capacity. Weighting by SOC span gives a long 76-point window proportionally more influence than a short 20-point window. A weighted average is used instead of a weighted median so one large, reliable session (such as a near-full charge) is properly reflected rather than being outvoted by several smaller sessions. When at least 9 retained regression fits agree on a non-zero intercept with a median absolute deviation within 1.5% of that vehicle's measured SOC-window capacity, Tesla Pulse publishes **Usable Capacity** as the BMS energy at indicated 100% SOC: SOC-window capacity plus that stable offset. Until this evidence threshold is met, it publishes the SOC-window capacity alone. All thresholds are measured from each vehicle's own telemetry; no model, nominal pack size, or fixed capacity is assumed.

#### Regression and charger-metered validation

The endpoint-based `capacity_kwh` formula above remains the SOC-window slope. Regression and charger-metered checks validate that slope and determine whether it can be converted into the BMS energy at indicated 100% SOC.
...
- **Implied reserve at 0% SOC.** Tesla Pulse compares each high-SOC observation (`EnergyRemaining` seen at 95%+ SOC) against what the SOC-window slope predicts at that SOC. The median gap is exposed as `implied_soc_zero_reserve_kwh`. A consistent positive value supports the affine BMS relationship `energy = reserve + capacity/100 × SOC`, rather than a pure proportion, and explains why an instantaneous ratio near 100% SOC reads higher than the SOC-window slope.
- **Intercept stability across sessions.** Tesla Pulse tracks `regression_intercept_kwh` across every retained window with a fitted regression, exposing `regression_intercept_sample_count`, `regression_intercept_median_kwh`, `regression_intercept_mad_kwh`, and `regression_intercept_stable`. At least 9 fits with a spread within 1.5% of the vehicle's own median SOC-window capacity are required before the offset is considered stable. Once stable, `estimated_usable_capacity` and Battery SOH use `soc_window_capacity_kwh + bms_energy_offset_kwh`; `bms_energy_at_100_percent_kwh` and `published_capacity_uses_bms_offset` make that basis explicit. Only up to the newest 12 accepted windows are retained, so this evidence remains recent rather than lifetime-blended.
