...
**SOH Confidence** is the SOC span of the most recent completed charge or discharge session, capped at 100%. A 20% confidence means the last accepted session spanned 20 SOC points; it does not mean the result is 20% accurate or inaccurate. Confidence reflects the size of the session backing the current estimate, not elapsed time or historical data volume. Each newly completed session immediately replaces the published estimate, so Usable Capacity and SOH always reflect your most recent charge or drive rather than being diluted by older sessions. Up to 12 recent sessions are still retained for diagnostics and as an outlier-rejection baseline.
...
The result is an estimate, not a Tesla BMS or service-mode measurement. Temperature, battery balancing, hidden buffers, and BMS recalibration can affect any single session's reading. Confidence reaches 100% when the most recent accepted session spans 100 SOC percentage points.
...
| **Usable Capacity** | kWh | Usable-capacity estimate from the most recently completed same-record SOC and remaining-energy session |
| **Battery SOH** | % | Estimated usable capacity divided by the configured usable capacity when new |
| **SOH Confidence** | % | SOC span of the most recent completed session, capped at 100% |
